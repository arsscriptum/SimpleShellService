...
//==============================================================================
//
//    Simple, Lightweight Service definition for Testing purposes
//
//==============================================================================
...
public:
   ServiceController(ServiceString name, DWORD access = SERVICE_ALL_ACCESS)
   {
      ...
      if (scHandle)
      {
         ...
         if (!srvHandle && SHOW_ERRORS)
         {
            std::wcout << "[ERROR] ServiceController OpenService returned NULL for " << srvName << std::endl;
            access = SERVICE_QUERY_STATUS | SERVICE_START | SERVICE_QUERY_CONFIG | SERVICE_STOP; //SERVICE_CHANGE_CONFIG 
            scHandle = ::OpenSCManager(nullptr, nullptr, SC_MANAGER_CONNECT);
            if (!srvHandle && SHOW_ERRORS)
            {
                std::wcout << "[ERROR] ServiceController OpenService returned NULL for " << srvName << std::endl;
   ...
   bool Delete()
   {
      ...
      if (srvHandle)
      {
         auto result = ::DeleteService(srvHandle);
         if (result != 0) {
             return true;
         }
         success = false;
